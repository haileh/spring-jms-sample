...
The JMS message broker Apache ActiveMQ is used embeddedly.
...
It simply uses 
```jmsTemplate.convertAndSend(<destination>, <message object>)``` to send message (the Product instance) to Jms queue.
...
This is a "Spring boot" application means a "stand-alone" application. It can be run in any environment with Java 8 installed.
